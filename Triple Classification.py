...
class Classification:
    ...
    def Subclassofclassification(self):
        ...
        for (s, p) in self.subclassofpos:
            if self.Bindis(self.conceptlist[s], self.conceptlist[p]) > 0:
                poslist.append((s, p))
            else:
                neglist.append((s, p))
...
if __name__ == '__main__':
    ...
    print("开始测试")
    test = Classification(conceptList, instanceList, relationList, instanceofListposTest, instanceofListnegTest, subclassofListposTest, subclassofListnegTest, tripleListposTest, tripleListnegTest,B_t = 1, B_r = 0.5)
